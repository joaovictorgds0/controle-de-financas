...
function createDeleteTransactionButton(id) {
  ...
  function renderTransaction(transaction) {
    ...
    const editButton = createEditTransactionBtn(transaction);
  ...
  // Função para salvar a transação (incluindo a edição)
  async function saveTransaction(ev) {
    ev.preventDefault();
    ...
    if (id) {
      // Se já houver um ID, significa que estamos editando uma transação existente
      const response = await fetch(`http://localhost:3000/transactions/${id}`, {
        method: 'PUT',
        body: JSON.stringify({ name, amount }),
        headers: {
      ...
      // Substituir a transação antiga pela editada na lista local
      const indexToRemove = transactions.findIndex((t) => t.id === id);
      transactions.splice(indexToRemove, 1, transaction);
      
      // Remover a transação antiga do DOM
      document.querySelector(`#transaction-${id}`).remove();
      
      // Renderizar a transação editada
      renderTransaction(transaction);
    } else {
      // Se não houver ID, significa que estamos criando uma nova transação
      const response = await fetch('http://localhost:3000/transactions', {
        method: 'POST',
        body: JSON.stringify({ name, amount }),
        headers: {
